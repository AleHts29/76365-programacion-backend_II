...
router.get("/:word([a-zA-Z%C3%A1%C3%A9%20]+)", async (req, res) => {
    try {
        ...
        // const findPet = await petsService.findByName(req.params.word)
        // if (!findPet) {
...
router.put("/:word([a-zA-Z%C3%A1%C3%A9%20]+)", async (req, res) => {
    try {
        const result = petsService.update({ _id: req.pet._id }, { isAdopted: false })
        console.log(result);
...
router.param("word", async (req, res, next, name) => {
    console.log("Buscando nombre de mascota: ", name);
    try {
